  ...
  for (const key in obj) {
    if (typeof obj[key] === "object") {
      usernames = usernames.concat(
        extractUsernames(obj[key] as UserList, `${key}.`)
      );
    } else {
...
export async function addSubscribers(profile: any) {
  ...
  for (const username of usernames) {
    ...
    if (!subscriberProfile) {
      console.log(
        `profile(@${profile.username}) #subscriber_not_found <${username}>`
      );
      continue;
    }
...
export async function addProfile(profile: any) {
  ...
  const city = await prisma.profile.findFirst({
    where: { placeId: profile.place, type: "City" },
  });
  if (city) {
    data.cityId = city.id;
  }
  ...
  if (profile.createdBy) {
    const createdBy = await prisma.account.findFirst({
    ...
    if (createdBy) {
      ...
      console.log(
        `profile(${profile.id} @${profile.username}): creator(${profile.createdBy}) not found`
      );
    }
  }
